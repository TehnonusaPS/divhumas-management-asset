...
export default function AuthenticatedLayout({ header, children }) {
    ...
    return (
        <div className="min-h-screen flex flex-col relative bg-slate-50 dark:bg-[#0A0A0A] text-slate-900 dark:text-white transition-colors duration-300 pb-16 md:pb-0">
            ...
            {/* Page Header (Simple, clean, fully transparent, no border-bottom) */}
            {header && (
                <header className="relative z-10 w-full bg-transparent py-8 transition-colors duration-300">
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 dark:bg-[#0c0c0c] border-t border-slate-200 dark:border-white/5 shadow-lg dark:shadow-[0_-10px_40px_rgba(0,0,0,0.8)] backdrop-blur-md transition-colors duration-300">
                <div className="flex justify-between items-center px-1 pb-safe pt-2 pb-2 w-full">
                    <Link href={route('dashboard')} className={`flex-1 flex flex-col items-center justify-center gap-1.5 p-1 transition-colors ${route().current('dashboard') ? 'text-[#E8192C]' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
                    ...
                    <Link href={route('aset')} className={`flex-1 flex flex-col items-center justify-center gap-1.5 p-1 transition-colors ${route().current('aset') && !route().current('aset.pencarian') ? 'text-[#E8192C]' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
                    ...
                    <Link href={route('aset.pencarian')} className={`flex-1 flex flex-col items-center justify-center gap-1.5 p-1 transition-colors ${route().current('aset.pencarian') ? 'text-[#E8192C]' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
                    ...
                    <Link href={route('pengaduan')} className={`flex-1 flex flex-col items-center justify-center gap-1.5 p-1 transition-colors ${route().current('pengaduan') ? 'text-[#E8192C]' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
                    ...
                    {(auth?.user?.roles?.includes('pimpinan') || auth?.user?.roles?.includes('super-admin')) && (
                        <Link href={route('statistik')} className={`flex-1 flex flex-col items-center justify-center gap-1.5 p-1 transition-colors ${route().current('statistik') ? 'text-[#E8192C]' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
                    ...
                    {auth?.user?.roles?.includes('super-admin') && (
                        <Link href={route('pengguna')} className={`flex-1 flex flex-col items-center justify-center gap-1.5 p-1 transition-colors ${route().current('pengguna') ? 'text-[#E8192C]' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
                    ...
                    {auth?.user?.roles?.includes('super-admin') && hasRoute('dev.components') && (
                        <Link href={route('dev.components')} className={`flex-1 flex flex-col items-center justify-center gap-1.5 p-1 transition-colors ${route().current('dev.components') ? 'text-[#E8192C]' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
                            <svg className="w-[22px] h-[22px]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                            <span className="text-[10px] font-medium leading-none">Dev</span>
                        </Link>
                    )}
                </div>
            </div>
